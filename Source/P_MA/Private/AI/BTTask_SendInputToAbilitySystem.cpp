...
EBTNodeResult::Type UBTTask_SendInputToAbilitySystem::ExecuteTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
	...
	auto FailSelectedSkill = [Monster]()
	{
		Monster->ResetSkillSelection();
	...
	if (!SkillManager || !AbilitySystemComponent) return FailSelectedSkill();

	if (!Monster->HasSelectedSkill()) return FailSelectedSkill();
	...
	if (SlotInputID == INDEX_NONE) return FailSelectedSkill();
	...
	if (!bActivated) return FailSelectedSkill();
